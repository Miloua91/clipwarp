...
// Function to handle changing the WebSocket address
  const handleWsAddressChange = async () => {
	...
	<View className="w-full flex flex-row space-x-1">
        <TextInput className="w-fit border-2 rounded pl-4 text-lg font-semibold" placeholder="Enter WebSocket address" onChangeText={text => setWsAddress(text)} value={wsAddress} />
	<AwesomeButton backgroundColor="white" width={60} onPress={handleWsAddressChange}>
...
export const webSocket = async () => {
  ...
  return new WebSocket(`ws://${wsAddress}:5678/`);
};
